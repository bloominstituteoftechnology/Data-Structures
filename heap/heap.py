''' parent to child formulas (use these in _sift_down):
    node i 
    left: 2i
    right: 2i + i
    --------------
    child to parent formulas (use these in _bubble_up):
    floor(i/2)
'''
...
class Heap:
  ...
  def insert(self, value):
    # append the value to the sotrage array
  ...
  def _bubble_up(self, index):
    # keep bubbling the element at the given index up the heap until it reaches a valid spot in the heap 
    ...
    while index // 2 > 0:
      ...
      if self.storage[index // 2] < self.storage[index]:
        ...
        self.sotrage[index // 2], self.storage[index] = self.storage[index], self.storage[index // 2]
